...
public class GameOfCodes2022A2 implements GameOfCodes2022Interface {
	...
	int findMaxLength(int blockIndex, int maxBlock, int currentLength) {
		...
		for (int i = c.indexByChar; i < allBlockByChar.get(c.charNumber).size(); i++) {
			...
			totalChar += n.size;
			int l3 = maxBlock - 1 > 0
					? findMaxLength(n.indexByAll + 1, maxBlock - 1, currentLength + totalChar)
					: currentLength + totalChar;
			R = Math.max(R, l3);
		}
		R = Math.max(R, findMaxLength(blockIndex + 1, maxBlock, currentLength));
		return R;
	}
